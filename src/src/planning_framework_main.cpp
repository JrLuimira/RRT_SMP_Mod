...
public:
    ...
    //!Calculate de distance between init and goal
    double calculateDGR();
...
void OnlinePlannFramework::planWithSimpleSetup()
{
    ...
    if (local_planner_name_.compare("PRMstar") == 0)
        ...
        local_planner = ob::PlannerPtr(new og::RRTMod(si_local, sum_linear_vel_, sum_angular_vel_, sum_distances_, sum_angles_,goal_odom_frame_,odom_data_));
    else
        local_planner = ob::PlannerPtr(new og::RRTMod(si_local, sum_linear_vel_, sum_angular_vel_, sum_distances_, sum_angles_,goal_odom_frame_,odom_data_));
        ...
        if (solved && simple_setup_global_->haveExactSolutionPath())
        {
            ...
            if (simple_setup_global_->haveExactSolutionPath() || distance_to_goal <= goal_radius_)
            {
                ...
                if (solved_local || simple_setup_local_->haveExactSolutionPath()) 
                {
                    ...
                    //if(simple_setup_global_ ->haveExactSolutionPath())
                    {
                        solution_found = true;

                    }

                    if ((solved_local || simple_setup_local_->haveExactSolutionPath() ) || distance_to_goal <= local_goal_radius_)
                ...
                {   //!Check it ASAP
                   ...
                   // OMPL_INFORM("AUN NO SE CREA");
                    //og::PathGeometric path_local_visual_aux = og::PathGeometric(simple_setup_local_->getSpaceInformation());
                    //visualizeRRTLocal(path_local_visual_aux);
                    //OMPL_INFORM("YA SE CREO");
